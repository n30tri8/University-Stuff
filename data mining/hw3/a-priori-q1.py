...
transactions.append(['A', 'B', 'D', 'G'])
transactions.append(['B', 'D', 'E'])
transactions.append(['A', 'B', 'C', 'E', 'F'])
transactions.append(['B', 'D', 'E', 'G'])
transactions.append(['A', 'B', 'C', 'E', 'F'])
transactions.append(['B', 'E', 'G'])
transactions.append(['A', 'C', 'D', 'E'])
transactions.append(['B', 'E'])
transactions.append(['A', 'B', 'E', 'F'])
transactions.append(['A', 'C', 'D', 'E'])
...
# output
output = open('.\\outputs\\q1\\section1_results.txt', 'w')
# a
output.write('a.\n')
for freq in frequent_item_sets[0]:
    output.write(str(freq) + '\n')
output.write('---------------------------------\n')
# b
output.write('b.\n')
for freq in frequent_item_sets[1]:
    output.write(str(freq) + '\n')
output.write('---------------------------------\n')
# c
output.write('c.\n')
for freq in frequent_item_sets[1]:
    if support_count(freq) >= 7:
        output.write(str(freq) + '\n')
output.write('---------------------------------\n')
# d
output.write('d.\n')
# rule generation
confidence_threshold = 1.0
confident_1 = generate_rules()
for key in confident_1:
    frequent_item = confident_1[key]['item set']
    for rule_set in confident_1[key]['rule list']:
