...
class Bidirectional:
    ...
    init_frontier = list()
    init_frontier_state_set = set()
    init_explored = set()
    goal_node = None

    def add_frontier(self, node):
        self.init_frontier.append(node)
        self.init_frontier_state_set.add(hash(node.state))
        self.stat_visited_nodes += 1
        self.stat_max_memory = max(self.stat_max_memory, len(self.init_frontier))

    def select_next_frontier(self):
        next_node = self.init_frontier.pop(0)
        self.init_frontier_state_set.remove(hash(next_node.state))
        return next_node
    ...
    def solution(self, node):
        self.stat(stat_expanded_nodes=len(self.init_explored))
        if not node:
            return False
        else:
            self.goal_node = node
