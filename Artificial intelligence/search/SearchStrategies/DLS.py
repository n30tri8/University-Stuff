...
class DLS:
    ...
    def __init__(self, cut_off):
        ...
        self.cut_off_error = False

        self.stat_expanded_nodes = 0
        self.stat_visited_nodes = 0
        self.stat_max_memory = 0
        self.stat_solution_depth = 0
        self.stat_solution_cost = 0

        self.frontier = list()
        self.explored = set()
        self.frontier_state_set = set()
        self.goal_node = None
    ...
    def solve(self, problem):
        ...
        if node.depth != self.cut_off:
            self.add_frontier(node)
        else:
            self.cut_off_error = True
        while True:
            ...
            for action in problem.actions(node.state):
                child = Node(problem.child_state(node.state, action[0]), node, action[0], action[1] + node.cost, node.depth + 1)
                child_hash = hash(child.state)
                if child_hash not in self.explored and child_hash not in self.frontier_state_set:
                    if problem.goal_test(child.state):
                        return self.solution(child)
                    if child.depth != self.cut_off:
