...
class AStar:
    ...
    def add_frontier(self, node):
        ...
        for idx, n in enumerate(self.frontier):
            if node.heuristic_value <= n.heuristic_value:
    ...
    def heuristic_function(node):
        norm = 0
        for idx, cell in enumerate(node.state.arrangement):
            # x/3 => row
            # x%3 => col
            norm += abs(cell/3 - idx/3)
            norm += abs(cell % 3 - idx % 3)
        return norm

    def solve(self, problem):
        node = Node(problem.initial_state, None, None, 0)
        node.heuristic_value = AStar.heuristic_function(node)
        ...
        while True:
            ...
            for action in problem.actions(node.state):
                ...
                child.heuristic_value = AStar.heuristic_function(child)
